...
def tucker_decomposition_conv_layer_VBMF(
        layer: nn.Module,
        normed_rank: List[int] = [0.2, 0.2],
) -> nn.Module:
    """Gets a conv layer,
    ...
    rank = estimate_ranks(layer)
    print(layer, "VBMF Estimated ranks", rank)
    if 0 in rank:
        return layer
    rank = [max(r, 1) for r in rank]
...
def decompose(module: nn.Module):
  ...
  for i in range(len(model_layers)):
      if (type(model_layers[i]) == nn.Sequential) \
              or (model_layers[i].__class__.__name__ == 'Conv'):
